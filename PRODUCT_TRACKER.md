...
- Next approval/unblock: user deploys the rules to the live project and confirms deny/allow in the Rules Playground; then proceed to Slice 3 — enforce `authorize()` on `/admin/*` and `/settings/*` route handlers.
