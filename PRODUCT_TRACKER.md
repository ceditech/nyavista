...
| Highest open risk | Authorization is fully built as libraries (F-030 1a–1c + enforcement glue) but not yet exercised on a real route (route wiring deferred to Phase 5/F-024). Firestore and Storage rules are deployed to `edx-nyavista` (both compiled successfully 2026-08-03) but not yet behaviorally spot-checked in the Rules Playground (R-012) |
| Build | PASS — `vinext build` (2026-08-02) |
| Tests | PASS — 55/55 Node tests (2026-08-03) and 94/94 isolated Playwright visual/interaction tests (2026-08-02); Auth Emulator integration executed 2026-08-02 (`pnpm test:auth:emulator` → 1/1 Node test PASS). Firebase Security Rules (`firestore.rules`, `storage.rules`) are verified by review only; per D-007 they are checked against direct Firebase at deploy time (console Rules Playground), not by an emulator test |
...
| F-034 | S2 | P6 | Firebase repositories, indexes, and storage boundaries | P0 | IN_PROGRESS | 20% | BUILD | Claude | Slice 1a done: `lib/domain.ts` schemas + `lib/repositories.ts` seam + in-memory reference; 6 tests. Firestore-backed impl, composite indexes, and storage boundaries next | CRITICAL | F-030 |
...
- Out of scope (Slice 1a): Firestore-backed repositories, composite indexes, storage boundaries, and their integration verification (later slices); the news-domain entities (articles/clusters/sources) that arrive with the ingestion phases.
- Dependencies: F-030 (done — role model, rules deployed). Chosen first because both F-024 and Phase 5 admin depend on it (D-009).
- Acceptance status: Slice 1a complete and verified — `pnpm exec tsc --noEmit`, `pnpm lint`, 6 `tests/repositories.test.ts`, and full `pnpm test` (build + 55/55 Node) PASS. Pure and provider-agnostic, so no network or emulator is needed.
...
- Build: added `lib/domain.ts` and `lib/repositories.ts` (functional factories, no classes/enums — Node type-stripping) and `tests/repositories.test.ts`, registered in the `test` script. Reuses `zod` (already a dependency); no new dependencies.
- Validate: tsc (strict) and ESLint PASS; 6 tests assert schema validation, per-uid singleton storage, owner-scoped collection isolation, owner-scoped remove, and write validation; full suite 55/55 with production build PASS.
- Evolve/deviations: entity scope is limited to F-024's needs now; news-domain entities come with ingestion. Firestore-backed implementations and indexes follow, verified against direct Firebase per D-007.
- Security/privacy/editorial/fairness: no I/O, secrets, or PII persistence introduced; the in-memory store is a test/dev mock. Owner-scoping is enforced in the repository API, not just the UI.
- Migration/rollback: additive only — delete `lib/domain.ts`, `lib/repositories.ts`, `tests/repositories.test.ts`, and revert the `package.json` test entry and tracker records. No data or schema migration.
- Next approval/unblock: Slice 1b — Firestore-backed repository implementations (client SDK, rules-enforced) + composite indexes in `firestore.indexes.json` + storage boundaries, verified against `edx-nyavista`. Then F-024 builds its UI on these interfaces.
...
| R-012 | Firebase Security Rules (`firestore.rules`, `storage.rules`) have no automated test coverage | Security/testing | Medium | High | HIGH | Rules are deny-by-default and mirror `lib/authz.ts`; per D-007 verify against direct Firebase at deploy time via the console Rules Playground before relying on them. Consider a CI rules check later (emulator on JDK 21+ or a service-account integration suite). Mitigation 2026-08-03: both `firestore.rules` and `storage.rules` deployed to `edx-nyavista` and compiled successfully on the server; behavioral Playground spot-check still pending | Rules edited without a Playground re-check; unauthorized read/write succeeds | — | OPEN |
