...
| Highest open risk | Authorization is fully built as libraries (F-030 1a–1c + enforcement glue) but not yet exercised on a real route (route wiring deferred to Phase 5/F-024), and the Security Rules are not yet deployed to or verified on the live project (D-007, R-012) |
...
- Known limitation: the Firestore/Storage rules have no automated test (D-007/R-012). They must be verified against the live project — deploy with `firebase deploy --only firestore:rules,storage:rules` (after pointing `.firebaserc` at the real project, not the reserved `demo-nyavista`) and exercise the deny/allow cases in the Firebase console Rules Playground.
...
- Next approval/unblock: two remaining items, both external to F-030's code. (1) Rules deploy — deploy path is wired (`pnpm deploy:rules` → `edx-nyavista` via the `.firebaserc` `prod` alias; Firebase MCP in `.mcp.json`, loads next session), but BLOCKED on the user re-authenticating the Firebase CLI (`firebase login --reauth`, interactive OAuth the agent cannot run; do not paste CI tokens into the repo). After deploy, verify deny/allow in the console Rules Playground. (2) Route wiring — Phase 5 (`/admin/*`) and F-024 (`/settings/*`) handlers call `authorizeRequest(request, permission)` when built; F-030 marks DONE once at least one real route enforces it and rules are deploy-verified.
