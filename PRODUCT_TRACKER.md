...
| Current phase | Phase 6/4 — F-030 authorization built + rules deployed & integration-verified; F-034 persistence at Slice 1b (Firestore repos); F-024 personalization Slice 1b (signed-in accessor + Saved UI, verified live); F-023 awaiting live QA |
...
| Visual baseline status | STALE — 72 committed baselines predate this session's `PRODUCT_TRACKER.md` changes, which drive the sidebar feature-count badge and the tracker view, so most workspace baselines now differ by content (plus apparent environment drift on non-sidebar routes). A deliberate, reviewed baseline refresh is pending (R-014). F-024's Saved view was verified live in-browser instead |
...
| P6 | Firebase persistence | IN_PROGRESS | Repositories, rules, indexes, emulator and permission tests pass | F-030 Slices 1a–1c + enforcement glue (`lib/rbac.ts`, `lib/server-session.ts`, `lib/authz.ts`, `lib/require-authorization.ts`, `firestore.rules` + `storage.rules`, 30 tests) and F-034 Slice 1a (`lib/domain.ts` + `lib/repositories.ts` + 6 tests); full 55/55, tsc/lint PASS | `firestore.rules` + `storage.rules` deployed to `edx-nyavista` (both compiled OK, 2026-08-03), Playground spot-check pending (D-007, R-012); enforcement not yet wired to a real route (deferred to Phase 5/F-024); F-034 repositories not started | User approved phase gate, 2026-08-02 |
...
| F-030 | 6 | Server-side RBAC and Firebase rules | P0 | IN_PROGRESS | Claude | Root STABLE, CONSTRAINTS, spec §6/§13, and Cloudflare Workers runtime inspected; sliced 1a model / 1b Workers session verification / 1c Firebase rules | Unauthorized access denied in UI and backend | Slices 1a–1c + enforcement glue: RBAC model + Workers JWKS verification + authorize()/RoleProvider seam + Firestore/Storage rules + `require-authorization` (token→verify→role→enforce→401/403); 30 unit tests + full 49/49 Node, tsc + lint PASS; rules verified by review, deploy-time check pending (D-007) | CRITICAL | Auth/data model | Security | Working tree |
...
| F-024 | S2 | P4 | Preferences, follows, bookmarks, and history | P1 | IN_PROGRESS | 40% | BUILD | Claude | Slices 1a–1b done: operations layer, `lib/firebase-client.ts` signed-in accessor + auth state, and the Saved (bookmarks) UI wired to live Firestore with sign-in-aware/loading/empty/error states (verified live in-browser, light + dark). Preferences/follows/history UI + baseline refresh next | HIGH | F-023, F-034 |
...
| F-030 | S2 | P6 | Server-side RBAC and Firebase rules | P0 | IN_PROGRESS | 85% | VALIDATE | Claude | Slices 1a–1c + enforcement glue done (code): RBAC model, Workers session verification, authorize()/RoleProvider seam, Firestore/Storage rules, `require-authorization`. Remaining: rules live deploy + Rules Playground check (D-007, user); wire enforcement into real routes as Phase 5/F-024 build them | CRITICAL | F-023, data model |
| F-034 | S2 | P6 | Firebase repositories, indexes, and storage boundaries | P0 | IN_PROGRESS | 50% | VALIDATE | Claude | Slices 1a–1b done: domain schemas, repository seam + in-memory reference, Firestore-backed repos, indexes file; verified by service-account integration suite (5/5 live) + 6 unit tests. Storage-backed media + broader entities later | CRITICAL | F-030 |
...
- Status / owner: IN_PROGRESS (85%) / Claude
- Approved outcome: enforce authorization on the server so unauthorized access is denied in both UI and backend; deliver in slices — 1a RBAC domain model, 1b Cloudflare Workers session verification + `authorize()`, 1c Firebase Security Rules, Slice 3 request-level enforcement glue (wired into routes as they are built).
...
- In scope (Slice 1c, delivered): `firestore.rules` and `storage.rules` encoding the same role model (deny-by-default; role from a verified `role` custom claim, else user/guest; owner-scoped, public-read, editorial, senior-editorial, and administrator tiers), plus the `firestore`/`storage` rules references in `firebase.json` for deployment.
- In scope (Slice 3, delivered): request-level enforcement glue in `lib/require-authorization.ts` — `createAuthorizer`/`authorizeRequest`/`sessionFromRequest` running token → verify → resolve role → enforce permission, an `AuthorizationDenied` error mapping to HTTP 401 (no/invalid session) vs 403 (under-privileged), and a fail-closed `defaultSessionVerifier` (Google JWKS + env project id). Injectable verifier keeps it unit-testable.
- Out of scope (still): wiring enforcement into concrete route handlers (deferred to Phase 5 `/admin/*` and F-024 `/settings/*`, which do not exist yet), Firebase Admin SDK (unsuitable on Workers), and role persistence/assignment (F-034 / F-033).
...
- Acceptance status: Slices 1a–1b + Slice 3 complete and verified — `pnpm exec tsc --noEmit`, `pnpm lint`, 30 F-030 tests (`rbac` 9, `server-session` 10, `authz` 5, `require-authorization` 6), and full `pnpm test` (build + 49/49 Node) PASS; crypto verification uses a locally generated RSA keypair and enforcement tests use an injected verifier, so neither needs network or emulator. Slice 1c rules are delivered and reviewed but NOT covered by an automated test (D-007): the emulator is not used, and automated live rules tests would need Admin-set custom claims (a service account, prohibited here); rules are verified against direct Firebase at deploy time via the console Rules Playground. Enforcement is proven at the library level but not yet exercised on a real route (no protected routes exist yet).
...
- Build: Slice 1a added `lib/rbac.ts` (seven roles, composed permission sets, `hasPermission`/`permissionsForRole`/`isRole`) and `tests/rbac.test.ts`. Slice 1b added `lib/server-session.ts` (JWT parse, RS256/`alg`/`kid` checks, Web-Crypto signature verify, and `iss`/`aud`/`exp`/`iat`/`auth_time`/`sub` claim validation; injected key resolver; Google JWKS resolver with `max-age` caching and rotation refresh; `readBearerToken`) and `lib/authz.ts` (`RoleProvider` seam, least-privilege and verified-custom-claims providers, `authorize`/`requirePermission`/`authorizeSession`, `AuthorizationError`), plus `tests/server-session.test.ts` and `tests/authz.test.ts`, registered in the `test` script. Slice 1c added `firestore.rules` and `storage.rules` (helpers mirror `lib/authz.ts`) and the `firestore`/`storage` rules refs in `firebase.json`. Slice 3 added `lib/require-authorization.ts` (`createAuthorizer`, `authorizeRequest`, `sessionFromRequest`, `AuthorizationDenied` + `toResponse`, `defaultSessionVerifier`) and `tests/require-authorization.test.ts`, registered in the `test` script. No enums/namespaces (Node type-stripping); no runtime dependencies added.
- Validate: tsc (strict) and ESLint PASS; 24 F-030 unit tests assert the RBAC matrix and axes, cryptographic accept/reject paths (expired, future-issued, wrong audience/issuer, missing subject, wrong signing key, unknown kid, `alg:none` downgrade, malformed), bearer parsing, the least-privilege and custom-claims providers, and `AuthorizationError`; plus Slice 3's 6 enforcement tests (guest 401 vs user 403 vs admin-via-claim allow, unverifiable-token-as-unauthenticated, `sessionFromRequest` fail-closed, and 401/403 Response mapping); full suite 49/49 with production build PASS. Two type-only fixes were needed for the new typed-array generics and `JsonWebKey.kid`. Slice 1c rules pass review only — no automated coverage (D-007); an emulator rules test was written then removed with its `@firebase/rules-unit-testing` dependency when the direct-Firebase decision was taken.
- Evolve/deviations: premium modeled as a consumer entitlement axis (D-006); server verification uses JWKS + Web Crypto rather than the Firebase Admin SDK, which is unsuitable on Cloudflare Workers (D-005); Firebase Rules are verified against direct Firebase at deploy time rather than the emulator (D-007). All recorded as intentional.
...
- Known limitation: the Firestore/Storage rules have no automated test (D-007/R-012). Both `firestore.rules` and `storage.rules` were deployed to `edx-nyavista` on 2026-08-03 (`pnpm deploy:rules`) and compiled successfully on the server; a behavioral Rules Playground spot-check is still recommended. Note: Storage rules deploy uses `--only storage` (not `storage:rules`, which firebase-tools reads as a named-bucket target and rejects).
- Migration/rollback: additive only — delete `lib/rbac.ts`, `lib/server-session.ts`, `lib/authz.ts`, `lib/require-authorization.ts`, `firestore.rules`, `storage.rules`, and the auth tests; revert the `firebase.json` rules refs, the `package.json` test entries, and tracker records. No data or schema migration.
- Next approval/unblock: (1) Firestore + Storage rules deployed to `edx-nyavista` on 2026-08-03 (both compiled successfully); a behavioral Rules Playground spot-check remains recommended. (2) Route wiring — Phase 5 (`/admin/*`) and F-024 (`/settings/*`) handlers call `authorizeRequest(request, permission)` when built; F-030 marks DONE once at least one real route enforces it and the deployed rules are spot-checked. Note: the Firebase MCP configured in `.mcp.json` did not load this session; deploys used the authenticated Firebase CLI instead.

### F-034 Firebase repositories, indexes, and storage boundaries

- Phase / epic: Phase 6 persistence and security
- Status / owner: IN_PROGRESS (50%) / Claude
- Approved outcome: a typed, validated persistence layer with owner/role boundaries and indexes, backing F-024 personalization and Phase 5 admin. Delivered in slices — 1a domain model + repository seam + in-memory reference; 1b Firestore-backed implementations + indexes + integration verification; later slices add storage-backed media and broader entities.
- In scope (Slice 1a, delivered): `lib/domain.ts` — Zod schemas + types for the F-024 entities (UserProfile, UserPreferences, Bookmark, Follow, HistoryEntry), strict and bounded, ISO-8601-UTC timestamps; `lib/repositories.ts` — `SingletonRepository`/`OwnedCollectionRepository` interfaces, owner-scoped in-memory reference implementations (the spec §9 mock), and per-entity factories. Boundary validation on every write.
- In scope (Slice 1b, delivered): `lib/firestore-repositories.ts` — Firestore-backed implementations of every repository interface over the modular `firebase/firestore` SDK, mapping to the exact paths the deployed rules protect (`users/{uid}`, `userData/{uid}/…`); `firestore.indexes.json` (wired into `firebase.json`, `pnpm deploy:indexes`; no composite index needed for the current single-collection ordered queries); and a live service-account integration suite (`tests/integration/firestore.integration.test.ts`, `pnpm test:integration`).
- Out of scope (still): storage-backed media repositories, and the news-domain entities (articles/clusters/sources) that arrive with the ingestion phases.
- Dependencies: F-030 (done — role model, rules deployed). Chosen first because both F-024 and Phase 5 admin depend on it (D-009).
- Acceptance status: Slices 1a–1b complete and verified — `pnpm exec tsc --noEmit`, `pnpm lint`, 6 `tests/repositories.test.ts`, and full `pnpm test` (build + 55/55 Node) PASS; the Firestore-backed repos and the deployed rules are verified together by the service-account integration suite (`pnpm test:integration` → 5/5 live on `edx-nyavista`: owner CRUD, cross-user denial, admin-collection role gate, fail-closed forged claim, profile owner/admin). The suite is opt-in (needs the service account + live project), not part of hermetic `pnpm test` (R-012).

#### STABLE record

- Scope/Think: F-024 and Phase 5 admin both list F-034 as a dependency; F-030 shipping unblocked it. The pure domain/repository layer was sliced first (like F-030 Slice 1a) so persistence contracts exist before any Firestore I/O.
- Assess Risk: risks are cross-user data leakage and unvalidated writes. Mitigations: repositories are keyed by `uid` (mirroring the deployed owner-only Firestore rules), and every write is Zod-parsed at the boundary; strict schemas reject unknown keys and malformed timestamps/country codes.
- Build: Slice 1a added `lib/domain.ts` + `lib/repositories.ts` (functional factories, no classes/enums — Node type-stripping) + `tests/repositories.test.ts`. Slice 1b added `lib/firestore-repositories.ts`, `firestore.indexes.json` (+ `firebase.json` wiring, `deploy:indexes`/`test:integration` scripts), and `tests/integration/firestore.integration.test.ts`. New dev dependency `firebase-admin` (mints custom tokens for synthetic test users; Node-only test tooling, never in the Workers runtime). Reuses `zod`/`firebase` (already dependencies).
- Validate: tsc (strict) and ESLint PASS; 6 unit tests (hermetic) assert schema validation and owner-scoped in-memory behavior; full suite 55/55 with production build PASS. The live service-account integration suite passed 5/5 against `edx-nyavista` on 2026-08-03, signing custom tokens locally with the service-account private key (via `cert(path)`) so no IAM Credentials API is required; it created only namespaced `itest-*` users/docs and deleted them all in teardown.
- Evolve/deviations: custom-token signing switched from `applicationDefault()` (which called the disabled IAM Service Account Credentials API) to `cert(GOOGLE_APPLICATION_CREDENTIALS path)` for local signing. Firestore-backed repos have no hermetic unit tests (they do I/O); the integration suite covers them, consistent with D-007's direct-Firebase verification.
...
- Phase / epic: Phase 4 authentication and personalization
- Status / owner: IN_PROGRESS (40%) / Claude
- Approved outcome: signed-in users personalize countries/regions/categories/topics/sources/language/depth/format, follow targets, bookmark stories, and see history — persisted per-user and enforced by the deployed rules. Delivered in slices — 1a operations + access layer; 1b signed-in accessor + first UI surface (Saved); later slices add preferences/follows/history UI and a baseline refresh.
- In scope (Slice 1a, delivered): `lib/personalization.ts` — pure operations encoding the F-024 domain rules (bookmarks/follows/history are idempotent by target) via `makeBookmark`/`makeFollow`/`makeHistoryEntry` + stable id helpers, and `createUserPersonalization(db, uid)` binding a signed-in user's Firestore instance to the full F-034 repository set.
- In scope (Slice 1b, delivered): `lib/firebase-client.ts` — lazy client Firebase accessor + `observeAuthUser` auth-state observation + `getPersonalizationFor(uid)` (dynamic imports, server-safe, fail-closed). And the Saved (bookmarks) view in `app/page.tsx`, wired to the existing "Saved stories" nav item: sign-in-aware, with loading/empty/error states and add/remove against live Firestore via the F-034 repos; `.saved-*` styles reuse the design tokens.
- Out of scope (still): preferences/follows/history UI surfaces, a save affordance on story cards/detail, and a deliberate Playwright baseline refresh — the next F-024 slices.
- Dependencies: F-023 identity, F-034 repositories (Slice 1b, integration-verified).
- Acceptance status: Slices 1a–1b complete — `pnpm exec tsc --noEmit`, `pnpm lint`, 3 `tests/personalization.test.ts`, and full `pnpm test` (build + 58/58 Node) PASS. The Saved view was verified live in the running app: it renders with the design system, resolves auth state (signed-out → "Sign in to save stories"), and themes consistently with existing views (its headings inherit color identically to shipped `.feed-heading` elements). Live signed-in CRUD remains gated on F-023 live QA (Email/Password enablement + an account). Formal visual baselines are STALE and deferred (R-014): the sidebar feature-count badge and tracker view are driven by this session's `PRODUCT_TRACKER.md` changes, so most baselines differ by content, not F-024 markup.
...
- Build: Slice 1a added `lib/personalization.ts` + `tests/personalization.test.ts`. Slice 1b added `lib/firebase-client.ts` (dynamic-import accessor + `observeAuthUser` + `getPersonalizationFor`), the `SavedStories` view + auth-state effect in `app/page.tsx` (wired to the existing "Saved stories" nav item; added `saved` to the view union, topbar title, and dock exclusion), `.saved-*` styles in `app/globals.css` (new selectors only — no existing rule changed), and `.claude/launch.json` for local preview. No new dependencies.
- Validate: tsc (strict) and ESLint PASS (fixed a `react-hooks/set-state-in-effect` finding by moving state updates into the async continuation); full suite 58/58 with production build PASS. Live in-browser check confirmed the Saved view renders, resolves signed-out state, and themes with the shared tokens. `pnpm visual:test` failures this session are content-driven (tracker-derived sidebar badge + tracker view) and environmental, not F-024 markup regressions (R-014).
- Evolve/deviations: kept the change additive/low-regression — reused the inert "Saved stories" nav item rather than restructuring the sidebar, and added only new CSS selectors, so existing views/baselines are untouched by F-024 markup. A save affordance on story cards is deferred to avoid churning the story-detail baseline.
- Security/privacy/editorial/fairness: bookmarks are owner-scoped and enforced by the deployed rules (integration-verified); the accessor fails closed when Firebase is unconfigured. Personalization interests are country-neutral and never affect editorial ranking (spec §4).
- Migration/rollback: additive only — delete `lib/firebase-client.ts`, revert the `app/page.tsx`/`app/globals.css` additions, `.claude/launch.json`, and the tracker records. No data migration (the Saved view only reads/writes the already-ruled `userData/{uid}/bookmarks`).
- Next approval/unblock: F-024 Slice 1c — preferences/follows/history UI surfaces on `createUserPersonalization`, a save affordance on story cards, and a deliberate reviewed Playwright baseline refresh (R-014) once tracker content settles. Live signed-in CRUD verification needs F-023 live QA (Email/Password + an account).
...
| R-012 | Firebase Security Rules coverage depends on an opt-in integration suite (not hermetic CI) | Security/testing | Low | High | MEDIUM | Mitigated 2026-08-03: rules deployed to `edx-nyavista` and behaviorally verified by the service-account integration suite (`pnpm test:integration`, 5/5 live) covering owner isolation, role gating, and the fail-closed forged-claim case. Residual: the suite needs the service account + live project so it is opt-in, not part of `pnpm test`; re-run it after any rules change and consider gating merges on it | Rules changed without re-running the suite; unauthorized read/write succeeds | — | MITIGATED |
| R-013 | Firebase Admin service-account key present in the working tree (`docs/service-account/`) | Security/secrets | Low | Critical | HIGH | Gitignored via `/docs/service-account/`, `*-firebase-adminsdk-*.json`, and `*serviceaccount*.json`; referenced from gitignored `.env.local` by path; never committed or placed in `NEXT_PUBLIC_*` (D-008). A location outside the repo would be safer still; rotate the key if it is ever committed or shared | Key appears in `git status`/a commit/a diff; or in any `NEXT_PUBLIC_*` var or client bundle | — | OPEN |
| R-014 | Visual-regression baselines are coupled to `PRODUCT_TRACKER.md` content (sidebar feature-count badge and the tracker view), so they drift whenever the tracker changes | UX/testing | High | Low | MEDIUM | Do a deliberate, reviewed `pnpm visual:baseline` refresh once tracker content settles — not blindly mid-session; consider decoupling the visual suite from live tracker content (fixed fixture) so product-UI regressions are not masked by content churn. F-024 Saved view verified live in-browser meanwhile | Baselines fail after any tracker edit; real UI regressions hidden among content diffs | — | OPEN |
...
| D-008 | 2026-08-03 | Firebase Admin service account is local-only tooling, referenced from `.env.local` by path, never committed | User added `docs/service-account/edx-nyavista-firebase-adminsdk-*.json` for admin operations (e.g. setting role custom claims). The Cloudflare Workers app does not use the Admin SDK (D-005), so this is not wired into the runtime | Key stays out of git (`.gitignore` covers `/docs/service-account/`, `*-firebase-adminsdk-*.json`, `*serviceaccount*.json`); `.env.local` (gitignored) references it via `GOOGLE_APPLICATION_CREDENTIALS` as a path, not inline contents; never place it in a `NEXT_PUBLIC_*` var (R-013) | User, 2026-08-03 | D-005; R-013; `.env.local` |
| D-009 | 2026-08-03 | Persistence as a repository seam: schemas + interfaces + in-memory reference now; Firestore-backed impl, indexes, and integration verification later (F-034) | F-024 and Phase 5 admin both need persistence; F-034 is the shared prerequisite, unblocked once F-030 shipped. Building the domain/repository layer first keeps it provider-agnostic and unit-testable | `lib/domain.ts` (Zod schemas) + `lib/repositories.ts` (interfaces + in-memory mock, spec §9) are owner-scoped by `uid` to mirror the deployed owner-only Firestore rules (F-030). Firestore-backed slices validate against direct Firebase (no emulator, D-007); F-024 builds on these interfaces | Claude, 2026-08-03 | F-034, F-024; D-007 |
| D-007 | 2026-08-03 | Verify Firebase Security Rules against direct Firebase (deploy + console Rules Playground), not the emulator | The Firestore/Storage emulators require JDK 21+ (this environment has JDK 17) and the team prefers direct Firebase. `@firebase/rules-unit-testing` is emulator-only; automated live rules tests would need Admin-set custom claims via a service account (prohibited here) | Removed the emulator rules test, the `test:rules:emulator` script, the Firestore/Storage emulator config, and the `@firebase/rules-unit-testing` dependency; kept `firestore.rules`/`storage.rules` and their `firebase.json` deploy refs. Rules carry no automated test (R-012) and are verified at deploy time. The Auth Emulator remains a retained fallback (D-003) | User, 2026-08-03 | F-030 (Slice 1c); R-012; refines D-003 for rules |
