...
| Highest open risk | Firebase Security Rules are written (F-030 1c) but not yet deployed to or verified against the live project, and no automated rules test exists (emulator declined, D-007; R-012); no route yet enforces `authorize()` (Slice 3) |
| Build | PASS — `vinext build` (2026-08-02) |
| Tests | PASS — 43/43 Node tests (2026-08-03) and 94/94 isolated Playwright visual/interaction tests (2026-08-02); Auth Emulator integration executed 2026-08-02 (`pnpm test:auth:emulator` → 1/1 Node test PASS). Firebase Security Rules (`firestore.rules`, `storage.rules`) are verified by review only; per D-007 they are checked against direct Firebase at deploy time (console Rules Playground), not by an emulator test |
...
| P6 | Firebase persistence | IN_PROGRESS | Repositories, rules, indexes, emulator and permission tests pass | F-030 Slices 1a–1c: `lib/rbac.ts` model, `lib/server-session.ts` (Workers JWKS verify), `lib/authz.ts` (authorize + RoleProvider seam), `firestore.rules` + `storage.rules`; 24 F-030 unit tests + full 43/43, tsc/lint PASS | Rules verified by review only, awaiting live deploy + Rules Playground (D-007, R-012); route-level enforcement (Slice 3) pending; F-034 repositories not started | User approved phase gate, 2026-08-02 |
...
| F-030 | 6 | Server-side RBAC and Firebase rules | P0 | IN_PROGRESS | Claude | Root STABLE, CONSTRAINTS, spec §6/§13, and Cloudflare Workers runtime inspected; sliced 1a model / 1b Workers session verification / 1c Firebase rules | Unauthorized access denied in UI and backend | Slices 1a–1c: RBAC model + Workers JWKS verification + authorize()/RoleProvider seam + `firestore.rules`/`storage.rules`; 24 unit tests + full 43/43 Node, tsc + lint PASS; rules verified by review, deploy-time check pending (D-007) | CRITICAL | Auth/data model | Security | Working tree |
...
| F-030 | S2 | P6 | Server-side RBAC and Firebase rules | P0 | IN_PROGRESS | 60% | BUILD | Claude | Slices 1a–1c done (code): RBAC model, Workers session verification, authorize()/RoleProvider seam, `firestore.rules`/`storage.rules`. Rules verified by review; live deploy + Rules Playground check pending (D-007). Slice 3 route enforcement next | CRITICAL | F-023, data model |
...
- Status / owner: IN_PROGRESS (60%) / Claude
...
- Out of scope (still): route/handler wiring and protected routes (Slice 3), Firebase Admin SDK (unsuitable on Workers), and role persistence/assignment (F-034 / F-033).
...
- Acceptance status: Slices 1a–1b complete and verified — `pnpm exec tsc --noEmit`, `pnpm lint`, 24 F-030 tests (`tests/rbac.test.ts` 9, `tests/server-session.test.ts` 10, `tests/authz.test.ts` 5), and full `pnpm test` (build + 43/43 Node) PASS; verification uses a locally generated RSA keypair, so it needs no network or emulator. Slice 1c rules are delivered and reviewed but NOT covered by an automated test: per D-007 the emulator is not used, and automated live rules tests would require Admin-set custom claims (a service account, prohibited here). Rules are verified against direct Firebase at deploy time via the console Rules Playground. Route-level enforcement remains unproven until Slice 3.
...
- Migration/rollback: additive only — delete `lib/rbac.ts`, `lib/server-session.ts`, `lib/authz.ts`, `firestore.rules`, `storage.rules`, and the auth tests; revert the `firebase.json` rules refs, the `package.json` test entries, and tracker records. No data or schema migration.
- Next approval/unblock: deploy path wired — `pnpm deploy:rules` targets `edx-nyavista` (`.firebaserc` `prod` alias); the Firebase MCP is configured in `.mcp.json` (loads next session). Deploy is BLOCKED on the user re-authenticating the Firebase CLI (`firebase login --reauth`, interactive OAuth the agent cannot run in a non-interactive session); do not paste CI tokens into the repo. After deploy + a Rules Playground check, proceed to Slice 3. Note: no protected routes exist yet (admin = Phase 5, settings = F-024, both NOT_STARTED), so Slice 3 will land as reusable server-enforcement glue and be wired into concrete routes when those features are built.
