...
| Highest open risk | Authorization is fully built as libraries (F-030 1a–1c + enforcement glue) but not yet exercised on a real route (route wiring deferred to Phase 5/F-024). Firestore rules are deployed to `edx-nyavista` (compiled successfully 2026-08-03) but not yet behaviorally spot-checked in the Rules Playground; Storage rules are undeployed pending Firebase Storage provisioning (R-012) |
...
- Known limitation: the Firestore/Storage rules have no automated test (D-007/R-012). `firestore.rules` was deployed to `edx-nyavista` on 2026-08-03 (`pnpm deploy:rules:firestore`) and compiled successfully on the server; a behavioral Rules Playground spot-check is still recommended. `storage.rules` is undeployed — Firebase Storage is not yet provisioned on the project; enable Storage in the console (creates the default bucket), then `pnpm deploy:rules:storage`.
...
- Next approval/unblock: (1) Firestore rules deployed to `edx-nyavista` on 2026-08-03 (compiled successfully); a behavioral Rules Playground spot-check remains recommended. (2) Storage rules — user enables Firebase Storage in the console, then `pnpm deploy:rules:storage`. (3) Route wiring — Phase 5 (`/admin/*`) and F-024 (`/settings/*`) handlers call `authorizeRequest(request, permission)` when built; F-030 marks DONE once at least one real route enforces it and both rule sets are deployed + spot-checked. Note: the Firebase MCP configured in `.mcp.json` did not load this session; deploys used the authenticated Firebase CLI instead.
